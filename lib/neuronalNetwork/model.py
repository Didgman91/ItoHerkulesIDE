...
def get_model_deep_speckle():
    ...
    # D1
    conv6 = Conv2D(128, 3, activation='relu', padding='same', kernel_initializer='he_normal')(merge5)
    ...
    # D2
    conv7 = Conv2D(64, 3, activation='relu', padding='same', kernel_initializer='he_normal')(merge6)
    ...
    # D4
    conv9 = Conv2D(16, 3, activation='relu', padding='same', kernel_initializer='he_normal')(merge8)
    print("conv9 shape:", conv9.shape)
    db9 = denseblock(x=conv9, concat_axis=3, nb_layers=3, growth_rate=16, dropout_rate=0.5)
    print("db9 shape:", db9.shape)
    
    # D5
    conv10 = Conv2D(8, 3, activation='relu', padding='same', kernel_initializer='he_normal')(db9)
    ...
    for x in range(pixelNumberX):
        for y in range(pixelNumberY):
            for c in range(channels):
                buffer += -(g[x, y] * fl.log(p[x,y,c]) + (1-g[x,y]) * fl.log(1-p[x,y,c]))
...
def getMetric(yTrue, yPred):
    ...
    for i in range(len(yTrue[0,:])):
        yTrue1d = np.concatenate((yTrue1d, yTrue[:,i]))
        yPred1d = np.concatenate((yPred1d, yPred[:,i,0]))
    
    buffer = pearsonr(yTrue1d, yPred1d) # c=0: like in the demo.py 
    
    print("Perason: {}".format(buffer))
